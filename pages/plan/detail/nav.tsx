...
export default function PlanNavigation({ query, plan }: Props) {
    ...
    useEffect(() => {
        if (mode) {
            setPlans(
                fakeData.metaData.plan.itineraries.filter(
                    (el) => el.pathType === mode
                )
            );
        } else {
        ...
        >
            <div className="relative basis-3/4">
                <div className="absolute z-10 flex h-full w-full flex-col justify-end space-y-3 bg-gray-200 p-1">
                    <button className=" mb-auto ml-auto rounded-2xl bg-slate-500 p-4  text-white">
            ...
            <div className="relative flex basis-1/4 flex-col border-r">
                <div className="absolute flex max-h-full min-h-full w-full flex-col ">
                    ...
                    <div className="space-x-2">
                        ...
                        >
                            {/* 버스 */}バス
                            {fakeData.metaData.requestParameters.busCount}
                        </ModeButton>
                        <ModeButton
                            color={`${mode === 1 && "blue"}`}
                            onClick={() => {
                        ...
                        >
                            {/* 지하철 */}地下鉄{" "}
                            {fakeData.metaData.requestParameters.subwayCount}
                        </ModeButton>
                        <ModeButton
                            color={`${mode === 3 && "blue"}`}
                            onClick={() => {
                        ...
                        >
                            {/* 버스 + 지하철 */}
                            バス + 地下鉄{" "}
                            {fakeData.metaData.requestParameters.subwayBusCount}
                        </ModeButton>
                    </div>
                        ...
                        {plans?.map((plan, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <div>
                                        <TimeText>
                                            {timeObj.hour && timeObj.hour}
                                        </TimeText>
                                        {timeObj.hour &&
                                            // "시간 "
                                            "時間 "}
                                        <TimeText>
                                            {timeObj.min && timeObj.min}
                                        </TimeText>
                                        {timeObj.min && //"분 "
                                            "分 "}
                                        <ColDivider />
                                        ...
                                        到着
                                        <ColDivider />
                                        {plan.fare.regular.totalFare}ウォン
                                    </div>
                                    <div>
                                        {plan.legs.map((leg) => {
                                            const transfortTime =
